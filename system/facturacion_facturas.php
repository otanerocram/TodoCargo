...
<section id="widget-grid" class="animated bounceInLeft">
	...
	<div class="row">
		...
		<article class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
			...
			<div class="jarviswidget jarviswidget-color-darken" id="wid-factura-cliente" data-widget-editbutton="false" data-widget-custombutton="false" data-widget-deletebutton="false" data-widget-colorbutton="false" data-widget-fullscreenbutton="false">
				<header>
										...
										<div class="row">
											...
											<div class="col col-md-5 col-xs-12">
												<label class="input"> <i class="icon-prepend fa fa-calendar"></i>
													<input type="text" id="nfFechaFactura" name="nfFechaFactura" placeholder="Fecha" class="text-center datepicker" data-dateformat='dd/mm/yy'>
													<b class="tooltip tooltip-top-left">Fecha de Nueva Factura</b>
												</label>
											</div>
										</div>
										<div class="row">
						...
						<form action="#" id="nfDetalle" class="">
							<fieldset>
								<div class="table-responsive">
									<table id="itemsFactura" class="table table-hover">
										<thead>
											<tr>
		...
		<article class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
			<!-- Widget ID (each widget will need unique ID)-->
			<div class="jarviswidget jarviswidget-color-darken" id="wid-factura-items" data-widget-editbutton="false" data-widget-custombutton="false" data-widget-deletebutton="false" data-widget-colorbutton="false" data-widget-fullscreenbutton="false">
				<header>
					<span class="widget-icon"> <i class="fa fa-comments"></i> </span>
					<h2>Últimas Facturas Creadas </h2>
				</header>
				<!-- widget div-->
				<div>
					...
					<!-- widget content -->
					<div class="widget-body no-padding">
						<form action="#" id="formBuscaFactura" class="smart-form">
							<fieldset>
								<div class="row">
									...
									<section class="col col-4 text-align-right">
										<button class="btn btn-success smart-btn" id="buscaFactura" type="submit">
											Busca Factura
										</button>
									</section>
								</div>
						...
						<div class="table-responsive custom-scroll"  style="height:700px; overflow-y: scroll;">
							<table id="ultimasFacturas" class="table table-striped table-bordered table-hover table-condensed" width="100%">
								<!-- -->
							</table>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#nfFechaFactura").datepicker().datepicker("setDate", new Date());
	    ...
	    $("#nfNotas").submit(function(){
	    	...
	    	while (factuItems.length) { factuItems.pop(); }

	    	var facturaBody 	= $("#itemsFact tr");
	    	var totalItems 		= facturaBody.length;
	    	
	    	var $row = facturaBody.closest("tr");
	    	var $tds = $row.find("td");
		    ...
		    for (j=0; j < totalItems; j++){
				...
				factuItems.push(data);
			}

			// console.log(factuItems);

		    var f = {
		    	oper:  				"add",
	    		fechaFactura: 		$("#nfFechaFactura").datepicker('getDate') / 1000,
	    		...
	    		subTotal: 			$("#nfSubTotal").html(),
	    		impuesto: 			$("#nfImpuesto").html(),
	    		total: 				$("#nfTotal").html(),
	    		notas: 				$("#nfNotes").val(),
	    		estado: 			"OK",
	    	...
	    	if (f.fechaFactura == 0){
	    		...
	    		$.smallBox({
					title : 	"Ups",
					content : 	"No ha especificado servicios para esta factura",
					color : 	"#B53636",
					timeout : 	4000,
					icon : 		"fa fa-bell swing animated",
				});
				$("#nfRemito").focus();
	    	} else if (f.codigoCliente.length <11){
				$.smallBox({
					title : 	"Ups",
					content : 	"Ese no es un RUC Válido",
					color : 	"#B53636",
					timeout : 	4000,
					icon : 		"fa fa-bell swing animated",
				});
	    		...
	    		console.clear();
	    		$.ajax({
					url: "webservice/_insertaFactura.php",
					data: f,
					method: "POST",
					timeout: 30000,
				    ...
				    success: function(datos){
						...
						$.each(resp,function(index,value){
							numFactura 	= resp[index].numFactura;
						});

						if (numFactura > 0){

							var onclick = "onclick='imprimirFactura(\""+numFactura+"\")'";

							var mensaje	=  "Código de la Factura: "+ numFactura + ", <br soft>";
							mensaje 	+= "¡Se ha creado la factura satisfactoriamente!<br>";
							mensaje 	+= "<h5>¿Desea Imprimir?</h5>";
							mensaje 	+= "<p class='text-align-right'>";
							mensaje 	+= "	<a href='javascript:void(0);' class='btn btn-default btn-sm' " + onclick + ">Imprimir Factura</a>";
			            	...
			            	setTimeout(function(){
								...
								total = 0;
								ajustaTotal();
								cargaUltimasFacturas(usrAgn, 0, 20);
			            	},300);
		...
		$.ajax({
		    ...
		    success: function(datos){
				...
				**/
				...
				$("#nfRemito").focus();
				$("#nfRemito").val("");
				$("#itemsFact").append(htmlTable).trigger('create', true);
	...
	function imprimirFactura(numFactura){
		window.open("webservice/_imprimirFactura.php?numFactura="+numFactura,'_blank');
	}

	function cargaUltimasFacturas(agencia, fecha, limit ){
		// lg("CargaFacturas. Agencia: " + agencia + ", Fecha: "+ fecha + ", Limite: " + limit);
		$.ajax({
	        url: "webservice/_cargaUltimasFacturas.php",
	        data: {
	        	agencia: 		agencia,
	        	fechaFactura: 	fecha,
	        	limit: 			limit
	        ...
	        success: function(datos){
	            ...
	            if ( resp[0].codigoFactura > 0){
		            ...
		            $.each(resp,function(index,value){
		            	cant++;
		            	var onclick = "onclick='imprimirFactura(\""+resp[index].codigoFactura+"\")'";
		            	
		            	var codFac 	= parseInt(resp[index].codigoFactura);
		            	htmlTable += "	<tr>";
		            	htmlTable += "		<td class='text-center'><a href='javascript:void(0);' "+onclick+">" + pad(codFac, 4) + "</a></td>";
	            	...
	            	$("#ultimasFacturas").empty();
	            	$("#ultimasFacturas").append(htmlTable).trigger('refresh', true);
	            	
	            	lg("Últimas facturas cargadas!. Agencia: " + agencia + ". Total: " + cant);
	            }else{
	            	htmlTable 	+= "<thead>";
		            htmlTable 	+= "	<tr>";
		            htmlTable 	+= "		<th class='text-center'><i class='fa fa-fw fa-barcode txt-color-blue hidden-md hidden-sm hidden-xs'></i> #</th>";
